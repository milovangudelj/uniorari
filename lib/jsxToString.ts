...
const stringifyObject = (object, opts?: any) => {
	let result;
...
const serializeItem = (item, options, delimit = true) => {
	let result;
...
const jsxToString = (component, options) => {
	const baseOpts = {
		displayName:
			component.type.displayName || component.type.name || component.type,
		ignoreProps: [],
		ignoreTags: [],
		keyValueOverride: {},
		spacing: 0,
	...
	if (component.props.children) {
		...
		if (Array.isArray(component.props.children)) {
			...
			componentData.children = serializeItem(
